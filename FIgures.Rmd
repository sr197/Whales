...
densc <- changing_area %>% filter(metric == "density") %>% ggplot(aes(x = year, y = density*100000, colour = protected_area)) + geom_line() +
  stat_poly_line() +
  stat_poly_eq(use_label(c("p", "R2"))) + geom_point() + theme_Publication + labs(y = expression("humpback denisty as individuals/100 km" ^2))+ scale_colour_manual(values = c("blue", "red"), labels = c("Great South Channel", "Stellwagen Bank")) + theme(legend.position=c(.42,.91))+
  labs(color=NULL) 
densc
...
areab <- whale_ct %>% ggplot(aes(x = year, y = ct)) + geom_point() + labs(y = "Central tendency of abundance (two-week period)", main = "Gulf of Maine")+ geom_smooth(aes(x = year, y = ct), method = "glm", colour = "blue") + theme_Publication
time1
